...
if [ -n "${SCANNER_CTRL_API_USERNAME}" ] && [ -n "${SCANNER_CTRL_API_PASSWORD}" ] && [ -n "${CLUSTER_JOIN_ADDR}" ] && [ -n "${CLUSTER_JOIN_PORT}" ]; then
  REGISTRY_ARG="${REGISTRY_ARG} -e CLUSTER_JOIN_ADDR=${CLUSTER_JOIN_ADDR} -e CLUSTER_JOIN_PORT=${CLUSTER_JOIN_PORT} -e SCANNER_CTRL_API_USERNAME=${SCANNER_CTRL_API_USERNAME} -e SCANNER_CTRL_API_PASSWORD=${SCANNER_CTRL_API_PASSWORD}"
fi
